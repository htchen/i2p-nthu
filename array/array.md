...
> 注意：
> array是從 0 開始，所以 int a[100] 的第一個變數是 a[0]， 最後一個是 a[99]， 並沒有 a[100]， 所以在使用array的時候要注意有沒有超過的問題
...
- 舉個例子來說，如果我今天宣告一個 int a[3] ，且沒有給初始值了話。那電腦可能會存成 a[0] = 451, a[1] = 4445, a[2] = 98432。（依電腦不一樣也有差別）  
- 歸零的方法大致分為 4 種  
...
```
---
講解：由結果可知，如果我們只對陣列的部份元素設定初值，則其餘的元素會被自動設為 0
