...
- 自己寫function， 在別的 function 裡面呼叫他
...
hi
```
 你會發現在 return 下面那一行的 printf 永遠不會執行到，這代表 function 的結束是在 return 而不是在 整個function 都執行完
...
ps.
標準差 reference ： [wiki - 標準差](https://zh.wikipedia.org/wiki/標準差)  
...
> Note:
> 但要注意，非 void function 一定要寫 return   
