...
如果`deleteNode`的參數的 type 是`Node*`使用方式可能像底下的程式寫法
```C
int main(void)
{
	Node *main_head;
...
變數宣告的時候，只要有星號，就表示是指標變數。假設我們寫`Node * p;`，代表`p` 是一個指標變數，可以用來記住某個 type 是`Node`的東西的位址。例如`p = (Node*) malloc(sizeof(Node));`。

至於`Node * * pp;`，`pp`也是指標變數，可以用來記住某個 type 是`(Node *)`的東西的位址。例如`pp = &p;`，`p`的 type 是`Node*`，所以`&p`的 type 是`(Node *) *`，剛好和`pp`的 type 一樣。所以可以做`pp = &p;`，用 `pp` 去記住 `p` 的位址。
```C
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
